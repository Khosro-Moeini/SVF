...
bool SVFUtil::matchType(const SVFType *lhs, const SVFType *rhs)
{
    ...
    {
        SVFStructType *lhsStruct = const_cast<SVFStructType*>(dyn_cast<SVFStructType>(lhs));
        SVFStructType *rhsStruct = const_cast<SVFStructType*>(dyn_cast<SVFStructType>(rhs));
...
bool SVFUtil::matchArgs(const CallICFGNode* call, const FunObjVar* callee)
{
    ...
    if (Options::ArgTypePrune())
    {
        ...
        if (!matchType(callRetType, calleeRetType))
            return false;
        for (u32_t i = 0; i < callee->arg_size(); i++)
        {
            const SVFType *callArgType = call->getArgument(i)->getType();
            const SVFType *calleeArgType = callee->getArg(i)->getType();
            if (!matchType(callArgType, calleeArgType))
                return false;
        }
    }
    return true;
